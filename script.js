...
const ingredientsLeft = document.querySelectorAll(
  ".arrow-up-left ~ .ingredients-box"
);
const ingredientsRight = document.querySelectorAll(
  ".arrow-up-right ~ .ingredients-box"
);
const additionalIngredientsLeft = document.querySelectorAll(
  ".additional-arrow-up-left ~ .ingredients-box"
);
const additionalIngredientsRight = document.querySelectorAll(
  ".additional-arrow-up-right ~ .ingredients-box"
);
...
arrowUpLeft.addEventListener("click", () => {
  ...
  additionalIngredientsLeft.forEach((ingredient, index) => {
    ingredient.style.transform = `translateY(-${
      (index + ingredientsLeft.length) * 220
    }px)`;
...
arrowUpRight.addEventListener("click", () => {
  ...
  additionalIngredientsRight.forEach((ingredient, index) => {
    ingredient.style.transform = `translateY(-${
      (index + ingredientsRight.length) * 220
    }px)`;
    ingredient.style.visibility = "visible";
    ingredient.classList.add("show");
  });
